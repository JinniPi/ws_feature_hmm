...
if __name__ == "__main__":
    ...
    # loadmodel
    model = helper.load_model("model/model_basic_not_idp_punt_normal.pickle")
    w_emission = model["emission"]
    w_transition = model["transition"]
    states = model["state"]
    vocab = model["vocab"]
    vocab_e = diction.covert_feature_to_array(vocab_feature_e, 2*len(vocab), list_stop_word, False)
    ...
    f = open("input_basic_not_idp_punt_normal.txt", "w")
